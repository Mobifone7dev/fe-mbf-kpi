...
export default function GDVComponent(props) {
  ...
  const getEmployee = async () => {
    setLoading(true);
    const result = await handleSearchEmployeeByArea(props.area, "%C7_%");
  ...
  const getExecKpiEmployee = async () => {
    ...
    const result = await handleGetExecKpiDLAEmployee(date, "%C7_%", props.area);
    const tempRes = await result.json();
