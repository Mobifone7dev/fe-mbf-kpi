...
export const exportKpiPlanExcelGDV = (finalData = []) => {
  ...
  const rows = finalData.map((item) => [
    ...
    item.TB_PTM_FWA_PLAN ?? 0,
    item.TB_PTM_FWAP_PLAN ?? 0,
    item.TB_C90N_C99N_PLAN ?? 0,
    item.TB_C90N_C99N_EXEC ?? 0,
    item.TB_GIA_HAN_DON_KY_PLAN ?? 0,
    item.TB_GIA_HAN_DON_KY_EXEC ?? 0,
    item.TB_TBTS_C1C_PLAN ?? 0,
    item.TB_TBTS_C1C_EXEC ?? 0,
    item.TB_TBTT_C1C_PLAN?? 0,
    item.TB_TBTT_C1C_EXEC?? 0,
    item.DTHU_N_1_PLAN ?? 0,
    item.DTHU_N_1_EXEC ?? 0,
